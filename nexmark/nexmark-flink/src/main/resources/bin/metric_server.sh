...
case $STARTSTOP in
    (start)
        ...
        java "${log_setting[@]}" -cp "$NEXMARK_HOME/lib/*:$FLINK_HOME/lib/*" com.github.nexmark.flink.metric.process.ProcessMetricReceiver &
    ;;

    (stop)
        PID="$(jps | grep ProcessMetricReceiver | awk '{print $1}')"
        kill -9 $PID
        echo "$PID ProcessMetricReceiver has been killed."
    ;;
esac
