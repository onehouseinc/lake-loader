...
case $STARTSTOP in
    (start)
        ...
        java "${log_setting[@]}" -cp "$NEXMARK_HOME/lib/*:$FLINK_HOME/lib/*" com.github.nexmark.flink.metric.process.ProcessMetricSender &
    ;;

    (stop)
        PID="$(jps | grep ProcessMetricSender | awk '{print $1}')"
        kill -9 $PID
        echo "$PID ProcessMetricSender has been killed."
    ;;
esac