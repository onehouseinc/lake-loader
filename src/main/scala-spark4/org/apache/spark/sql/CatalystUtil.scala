...
// Spark 4.0 moved the concrete Dataset implementation (and its `logicalPlan` /
// `ofRows` internals) into `org.apache.spark.sql.classic`; the `org.apache.spark.sql.Dataset`
// referenced by the `DataFrame` alias is now an abstract, Connect-compatible API type.
import org.apache.spark.sql.classic.{Dataset => ClassicDataset}

object CatalystUtil {
  def partitionLocalLimit(df: DataFrame, n: Int): DataFrame = {
    val classicDf = df.asInstanceOf[ClassicDataset[Row]]
    val originalPlan = classicDf.logicalPlan
    originalPlan match {
      ...
      case Repartition(numPartitions, _, _) =>
        ...
        ClassicDataset.ofRows(classicDf.sparkSession, partitionLocalLimitPlan)
